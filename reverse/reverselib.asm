...
.pushBytes:     					; push bytes on stack in reverse
	mov bx, word [Buff+edx] ; copy 2 bytes from buffer offset into ax
	xchg bh,bl							; exchange halves
	push bx									; push last 2 bytes at end of buffer
	add edx,2								; add 2 to edi for traversing the buffer
...
.popBytes:
	...
	sub edx,2								; decrease counter by 2 to know when stack is empty
